...
func DefaultOptions() Options {
	return Options{
		...
		ConnectionTimeouts: 15 * time.Second,
		SetClientName:      true,
...
func NewRedisExporter(redisURI string, opts Options) error {
	...
	if keys, err := parseKeyArg(opts.CheckKeys); err != nil {
		return fmt.Errorf("couldn't parse check-keys: %#v", err)
	} else {
		log.Debugf("keys: %#v", keys)
	}

	if singleKeys, err := parseKeyArg(opts.CheckSingleKeys); err != nil {
		return fmt.Errorf("couldn't parse check-single-keys: %#v", err)
	} else {
...
func (e *Exporter) scrapeRedisHost(ch chan<- prometheus.Metric) error {
	...
	if err != nil {
		log.Errorf("connectToRedis( %s ) err: %s", e.redisAddr, err)
		return err
	}
	defer c.Close()
